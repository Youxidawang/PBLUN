...
class ASTE(pl.LightningModule):
    def __init__(self, hparams, data_module):
        ...
        self.model = PBLUNModel.from_pretrained(self.hparams.model_name_or_path, config=self.config)
    ...
    def load_model(self):
        ...
        self.model = PBLUNModel.from_pretrained(dir_name)
    ...
    def training_step(self, batch, batch_idx):
        ...
        outputs = self(**batch)
        loss = outputs['table_loss_S'] + outputs['table_loss_E'] + outputs['pair_loss']+ outputs['aspect_preds_loss'] + outputs['opinion_preds_loss']
    ...
    def configure_optimizers(self):
        ...
        diff_part = ["cls_linear_aspect", "cls_linear_opinion"]
        def has_keywords(n, keywords):
            return any(nd in n for nd in keywords)

        optimizer_grouped_parameters = [
            {
                'params': [p for n, p in self.model.named_parameters() if not has_keywords(n, no_decay) and not any(nd in n for nd in diff_part)],
                'lr': self.hparams.learning_rate,
                'weight_decay': 0
            },
            {
                'params': [p for n, p in self.model.named_parameters() if has_keywords(n, no_decay) and not any(nd in n for nd in diff_part)],
                'lr': self.hparams.learning_rate,
...

            {
                'params': [p for n, p in self.model.named_parameters() if
                           not has_keywords(n, no_decay) and any(nd in n for nd in diff_part)],
                'lr': self.hparams.cls_lr,
                'weight_decay': 0
            },
            {
                'params': [p for n, p in self.model.named_parameters() if
                           has_keywords(n, no_decay) and any(nd in n for nd in diff_part)],
                'lr': self.hparams.cls_lr,
                'weight_decay': self.hparams.weight_decay
            }
        ]
